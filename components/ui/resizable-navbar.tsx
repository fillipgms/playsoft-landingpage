...
export const Navbar = ({ children, className }: NavbarProps) => {
    ...
    return (
        <motion.div
            ...
            className={cn(
                "fixed inset-x-0 left-0 top-20 z-40 w-full",
                className
            )}
        >
        ...
        >
            {items.map((item, idx) => (
                <a
                    ...
                    onClick={onItemClick}
                    className="relative px-4 py-2 text-[#F6F7F9]"
                    key={`link-${idx}`}
                    href={item.link}
                >
                    {hovered === idx && (
                        <motion.div
                            layoutId="hovered"
                            className="absolute inset-0 h-full w-full rounded-full bg-[#1F8FFF]/10"
                        />
                    )}
    ...
    return (
        ...
        >
            <img
                ...
                height={30}
            />
            <span className="font-medium  text-[#F6F7F9]">Playsoft</span>
        </Link>
    );
};
